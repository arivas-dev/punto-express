...
export const useFilters = ({ products }: { products: Product[] }) : useFilters => {
    ...
    const productsFiltered: Product[] = productsSorted.filter((product: Product) => {
        return product.price >= rangeFilter[0] && product.price <= rangeFilter[1]
    })
